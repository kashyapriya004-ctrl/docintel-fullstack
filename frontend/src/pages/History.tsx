...
import { Clock, Trash2, ChevronDown, ExternalLink, Search } from "lucide-react";
    ...
    <div className="container py-12 md:py-20 max-w-3xl page-enter">
      ...
      <div className="flex items-center justify-between mb-10">
        ...
        {history.length > 0 && (
          <Button
            ...
            className="font-sans text-muted-foreground gap-1.5 hover:text-destructive hover:border-destructive/50 transition-colors"
          >
      ...
      {history.length === 0 ? (
        <div className="text-center py-24 page-enter">
          ...
          <Button asChild size="sm" className="bg-sienna hover:bg-sienna/90 text-sienna-foreground font-sans gap-2 btn-lift">
            <Link to="/search">
        ...
        <div className="flex flex-col gap-4">
          {history.map((entry, i) => (
            <div
              key={entry.id}
              className="history-card relative bg-card border rounded-xl overflow-hidden"
              style={{ animationDelay: `${i * 0.06}s` }}
            >
              {/* Top accent strip */}
              <div className="h-1 bg-gradient-to-r from-primary/40 via-accent/60 to-gold/40" />

              <div className="p-6">
                {/* Delete button */}
                <button
                  onClick={(e) => { e.stopPropagation(); setDeleteTarget(entry.id); }}
                  className="absolute top-5 right-5 p-1.5 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all duration-200"
                  title="Delete"
                >
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <p className="font-display font-semibold text-primary group-hover:text-accent transition-colors duration-200">
                      {entry.question}
                    </p>
                    <ChevronDown
                      className={`h-4 w-4 text-muted-foreground flex-shrink-0 mt-0.5 transition-transform duration-300 ${
